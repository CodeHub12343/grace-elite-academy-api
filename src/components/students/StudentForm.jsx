...
export function StudentForm({ onSubmit, defaultValues }) {
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm({
    resolver: zodResolver(schema),
    defaultValues: defaultValues || { name: '', className: '', status: 'active' },
  })
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
      ...
      <div>
        ...
        <input className="w-full px-3 py-2 rounded-md border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900" {...register('className')} />
        {errors.className && <div className="text-xs text-red-500 mt-1">{errors.className.message}</div>}
      </div>
      <div>
